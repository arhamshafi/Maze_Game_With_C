...
void showMenu()
{
    ...
    cout << "5. Search Area\n";
    cout << "6. Add Resources\n";
    cout << "7. Display Highest Priority Area\n";
    cout << "8. Exit\n";
    setColor(CLR_DEFAULT);
    cout << "Enter choice: ";
}
...
void setupResourcePool()
{
    ...
    totalFood = readIntSafe();
    cout << "Total Water Bottles: ";
    totalWater = readIntSafe();
    cout << "Total Medicine Kits: ";
    totalMedicine = readIntSafe();
}
...
void addArea()
{
    ...
    do
    {
        ...
        {
            setColor(CLR_ERROR);
            cout << "Error: Children + Elderly (" << (children + elderly) << ") cannot exceed total Population ("
                 << population[i] << "). Re-enter.\n";
            setColor(CLR_DEFAULT);
        }
...
void viewAreas()
{
    ...
    cout << left << setw(15) << "Area" << setw(12) << "Population" << setw(10) << "Damage" << setw(10) << "Children"
         << setw(10) << "Elderly" << setw(12) << "MedNeed";
    ...
    for (int i = 0; i < totalAreas; i++)
    {
        ...
        cout << left << setw(15) << areaName[i] << setw(12) << population[i] << setw(10) << damageStr << setw(10)
             << childrenCount[i] << setw(10) << elderlyCount[i] << setw(12) << medStr;
        if (priorityCalculated)
...
void calculatePriority()
{
    ...
    for (int i = 0; i < totalAreas; i++)
    {
        priorityScore[i] = population[i] + (damageLevel[i] * 10) + (medicalNeed[i] * 8) + (childrenCount[i] * 2) +
                           (elderlyCount[i] * 2);
    }
...
void allocateResources()
{
    ...
    for (int i = 0; i < totalAreas; i++)
    {
        if (totalFoodNeed > 0)
        {
            foodGiven[i] = (totalFood >= totalFoodNeed) ? foodNeeded[i]
                                                        : (int)((long long)totalFood * foodNeeded[i] / totalFoodNeed);
        }
        if (totalWaterNeed > 0)
        {
            waterGiven[i] = (totalWater >= totalWaterNeed)
                                ? waterNeeded[i]
                                : (int)((long long)totalWater * waterNeeded[i] / totalWaterNeed);
        }
        if (totalMedNeed > 0)
        {
            medGiven[i] = (totalMedicine >= totalMedNeed)
                              ? medNeeded[i]
    ...
    for (int i = 0; i < totalAreas; i++)
    {
        ...
        setColor(CLR_SUCCESS);
        cout << "\nArea: " << areaName[i] << " (Priority Score: " << priorityScore[i] << ")\n";
        setColor(CLR_DEFAULT);
    ...
    cout << "\n--- Remaining Resources ---\n";
    setColor(CLR_DEFAULT);
    cout << "Food: " << totalFood << " | Water: " << totalWater << " | Medicine: " << totalMedicine << endl;
}
...
void searchArea()
{
    ...
    for (int i = 0; i < totalAreas; i++)
    {
        if (areaName[i] == target)
        {
            ...
            {
                setColor(CLR_SUCCESS);
                cout << "Resources Allocated -> Food: " << allocatedFood[i] << ", Water: " << allocatedWater[i]
